...
func _build_ui() -> void:
	...
	for upgrade_id in META_UPGRADES:
		...
		var cost_text: String = "" if maxed else Locale.t("%dG" % cost, "%d金" % cost)
	...
	_info_label.text = Locale.t("Select an upgrade to see details", "选择一项升级以查看详情")
	add_child(_info_label)

	# Start Voyage button
	var start_btn := Button.new()
	start_btn.text = Locale.t("Start Voyage", "出发远航")
...
func _on_upgrade_pressed(upgrade_id: String) -> void:
	...
	if GameState.purchase_meta_upgrade(upgrade_id, cost):
		_info_label.text = Locale.t("Upgrade purchased!", "升级成功！")
		_update_gold_display()
		_refresh_buttons()
